...
| **D18** | `/status` 결과 카드를 **선사(등록된 배) 단위로 묶고**, 선단이면 카드 안에 배별 개별 알림 토글 | Claude Design 목업(`선박 관리.dc.html`) 재확인 후 반영. 기존에도 배별(ship_name 단위) 알림 등록 자체는 있었다 - 이번 건 화면 재구성이지 새 로직이 아니다. `sortRows()`가 만든 정렬된 평면 배열에서 `registered_name`이 처음 등장하는 순서를 그대로 카드 순서로 쓰는 안정 그룹핑(`groupByCompany`), 대표 행은 `statusRank` 최우선 순위로 고른다(`bestOf`). 백엔드 무변경, `Watch.toggle`/`canWatch`/`isWatched` 그대로 재사용(시각만 종 아이콘 압축 토글로 추가: `buildWatchToggle(r, compact)`). 목업의 선장/출항시각/찜 필드는 파서가 안 주는 데이터라 스키마를 늘리지 않고 구현하지 않았다. |
| **D19** | `/status`의 알림 토글을 **"빈자리 알림 토글 스펙"**(Claude Design)대로 재설계: 항상 아이콘(🔔/🔕)+글자 라벨을 보여주고(꺼짐/비활성 상태도 빈 동그라미가 아니라 명시적으로 표시), 선단 카드엔 **전체 알림 마스터 토글**(all-on/partial/none 3상태 + n/N 카운터) 추가 | D18에서 새로 만든 압축 종 토글이 "꺼짐 상태를 빈 동그라미로만 표현해 눌러보기 전엔 기능을 알 수 없다"는 스펙의 지적을 반영. `buildWatchToggle`이 이제 `Watch.canWatch(r)`가 false여도 `null`을 반환하지 않고 `disabled` 상태로 항상 그린다(이유는 title/aria-label로 설명). `syncWatchRowVisual`/`buildWatchToggle`을 `.bell-toggle`/`.bell-toggle-icon`/`.bell-toggle-label` 클래스로 교체(구 `.watch-row`/`.watch-check-icon`/`.watch-label-text` 완전 대체 - `grep`으로 잔여 참조 없음 확인). 마스터 토글(`buildFleetMasterToggle`)은 새 API를 안 만들고 기존 `syncWatchRowVisual`+`Watch.toggle`을 체크박스마다 순차 호출한 뒤 `renderRows(lastRenderedRows)`로 다시 그린다. on/partial/none 카운트 로직은 별도 Node 스크립트로 단위 검증(4케이스: 전체on/일부on/전체off/1척뿐). 스펙의 정확한 oklch 색 토큰·450ms 링 애니메이션(`prefers-reduced-motion` 가드)·모바일 420px 이하 아이콘 전용 44px 변형 그대로 반영. |
| **D20** | `/status` 선단 카드는 배별로 **각자의 어종**을 보여준다(대표 행 하나로 뭉뚱그리지 않음). `scrape.yml`의 GitHub Actions `schedule` cron은 **변경 직후 반영이 크게 지연된다**는 것을 실측으로 재확인 - 진단은 `workflow_dispatch` 수동 실행으로 한다 | (1) 레드헌터 선단처럼 같은 선사 안에서도 배마다 어종이 다른데(주꾸미/문어/갈치 등) 카드가 대표 배 하나의 어종만 보여줘서 나머지 배 정보가 사라졌었다. `fleet-ship-row`마다 자기 어종을 보여주고, 카드 하단 공통 어종 태그는 오해 소지가 있어 선단일 땐 생략한다. (2) "체크 기록 로그가 하루가 지나도 안 쌓인다"는 신고를 진단하며 `scrape.yml`의 cron을 5분 간격으로 임시로 바꿔 실측했는데, **19분이 지나도 새 스케줄 실행이 한 번도 안 돌았다** - GitHub Actions가 워크플로 파일의 cron 변경을 반영하는 데 스케줄 주기와 무관하게 상당히 지연시키는 것으로 보인다(D14/D15에서 keepalive 워크플로로 이미 겪은 것과 같은 부류의 문제가 scrape.yml에도 있음 - "매시간 1회는 안정적"이라던 이전 결론은 재검토 필요). 그래서 즉시 결과가 필요한 진단은 cron을 바꾸는 대신 이미 있는 `workflow_dispatch`(수동 실행 버튼)를 쓰기로 했다 - 실제로 수동 실행 1회로 곧바로 확인됐다: 체크 기록 로그 자체는 정상 동작(`WatchCheckLog` 기록·`/api/watches/history` 조회·화면 표시 전부 정상, 라이브 테스트 감시로 실증). 사용자가 신고한 원래 7건은 이후 실제로 로그가 쌓여있는 것이 확인돼(스케줄이 예상보다 뜸하게 도는 것 + 확인 시점 타이밍 문제였을 뿐) 별도 코드 버그는 아니었다. 부수적으로: 표 레이아웃도 같이 정리 - 배 이름 칸이 `1fr`(남는 공간 전부)라 넓은 화면에서 수백px로 늘어나 다른 칸(특히 날짜 64px)이 오히려 좁아 줄바꿈되던 문제를 고쳤다(칸 폭을 고정/상한으로, 날짜는 `YYYY-MM-DD` 대신 `M/D`로 축소). |
| **D21** | 스크래핑 트리거를 GitHub Actions에서 **cron-job.org(외부 cron 서비스)로 완전히 이전**, `scrape.yml` 파일 자체를 삭제 | D20 진단 이후에도 실제 체크 기록 로그 시각을 모아보니 간격이 38분~6시간으로 여전히 불규칙했다("빈자리 나면 바로 알림"이 목적인 서비스에 6시간 지연은 치명적). 근본 원인이 GitHub Actions `schedule` 트리거 자체의 신뢰성 문제로 재확인된 이상, D15(keepalive를 UptimeRobot으로 옮긴 것)와 같은 해법을 그대로 적용: cron-job.org가 `POST /api/scrape/run`을 `X-Scrape-Token` 헤더로 인증해 30분 간격으로 직접 호출한다(사용자가 직접 가입·설정, 토큰 값은 대화/코드에 남기지 않고 Render 환경변수에서 그대로 복사해 cron-job.org 폼에 입력). GitHub Actions `schedule`과 외부 cron을 동시에 두면 앱에 동시실행 방지 잠금이 없어 같은 배를 두 번 긁을 위험이 있어 `schedule`을 먼저 제거했었고, 이어서 cron-job.org 자체에도 수동 실행 버튼이 있어 GitHub Actions의 `workflow_dispatch`(수동 진단용으로 남겨뒀던 것)까지 완전히 중복이 되어 `scrape.yml` 파일을 통째로 삭제했다. 이제 GitHub Actions는 이 프로젝트의 스크래핑 트리거에 전혀 관여하지 않는다. |
