...
- 이 환경(Windows Git-Bash)에서 `lsof`가 포트 점유 프로세스를 못 찾는다. 로컬 서버
  ...
  겪음).
- 로컬 서버가 `debug=False`(기본값)로 뜨면 Flask/Jinja의 `TEMPLATES_AUTO_RELOAD`가
  꺼져 있어, `.html` 템플릿을 고쳐도 **서버를 껐다 켜지 않으면 컴파일 캐시가 그대로
  쓰인다** — 파일은 바뀌었는데 브라우저/Playwright 결과는 예전 그대로 나와서 "고쳤는데
  왜 반영이 안 되지"라는 혼란을 겪었다(실제로 겪음, 몇 차례의 프런트 버그 재현·검증이
  이걸 몰라서 잘못된 결론을 낼 뻔했다). **status.html 등 템플릿을 수정한 뒤 로컬에서
...
5. **토큰 사용은 항상 최적화 대상이다(사용자가 두 차례 명시적으로 강조 — 특정
   작업 한 번이 아니라 이 프로젝트에서 일하는 내내 지켜야 하는 기본 태도)**.
   가장 먼저 손댈 지점은 검증/테스트 실행 비용이다. 회귀 테스트 자체는 계속
   철저히 한다("초록불에서만" 원칙은 안 바뀐다) — 아끼는 대상은 테스트가
   아니라 **테스트를 도는 방식과 시각 검증 빈도**다.
   - `pytest` 전체 스위트는 **커밋 직전 딱 1번**만 돈다. 그 전 반복 작업 중에는
     건드린 영역의 테스트 파일만 타겟으로 돌린다(예: `pytest tests/test_admin_port_management.py -q`).
     전체 스위트를 여러 번 돌려서 나오는 동일한 통과 결과를 반복 확인하지 않는다.
   - **Playwright 등 브라우저 서브에이전트 시각 검증은 세션당/기능 묶음당 되도록
     1회로 모아서** 요청한다 — 작은 수정 하나 고칠 때마다 새 에이전트를 또 띄우지
     않는다. 정말 재검증이 필요하면(예: 방금 고친 버그 하나만 확인) 새 에이전트를
     새로 만들지 말고 **직전 에이전트를 `SendMessage`로 이어서** 쓰거나, 그것도
     과하면 `curl`/`getComputedStyle` 같은 가벼운 확인으로 대체할 수 있는지 먼저
     따져본다 — 브라우저 왕복 없이 API 응답이나 HTML 마커만으로 확인 가능한
     것(예: 새 필드가 응답에 들어있는지)은 Playwright를 쓰지 않는다.
   - **백그라운드로 돌린 Bash/Agent 작업은 완료되면 자동으로 알림이 온다 -
     그걸 기다리려고 `ScheduleWakeup`으로 짧은 간격 재기상을 걸지 않는다**(도구
     설명에도 명시돼 있지만 이번 세션에 반복 위반했다 - 매 재기상마다 컨텍스트가
     다시 채워지며 그 자체가 비용이다). 정말 기다릴 게 없으면 그냥 턴을 끝내고
     알림이 올 때까지 기다린다.
