...
알려진 한계: 대체공휴일 규정은 "설날/추석 연휴가 토요일·일요일과 겹칠 때"뿐
아니라 "다른 공휴일과 겹칠 때"도 대상으로 삼는데, 이 함수는 요일 겹침만
구현했다(날짜 자체가 겹치는 경우는 안 봄). 이런 겹침은 드물게 발생한다(예:
2028년 추석과 개천절이 같은 날) - 그런 해엔 이 함수가 대체공휴일을 놓칠 수
있으니 정부 발표를 직접 확인해야 한다. 같은 날짜에 공휴일이 두 개 겹치면
이름은 '·'로 합쳐서 보여준다(위 2028년 사례처럼 놓치지 않고 알아볼 수 있게).
"""
...
def kr_holidays_for_year(year: int) -> Dict[str, str]:
    ...
    for span, name in ((seollal_span, '설날'), (chuseok_span, '추석')):
        if any(_is_weekend(d) for d in span):
