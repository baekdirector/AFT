...
#: 이용자가 친구 5~6명 안쪽인 취미 규모라 수집량을 작게 유지하는 것이 목적이다.
#: 감시 대상만 주기 수집하므로 이 숫자가 곧 스케줄러 부하다. 원래 5였는데,
#: 그 근거였던 "GitHub Actions 무료 분(private repo)"이 이 리포가 public이라
#: 해당 없어져서 20으로 올렸었다(D16). 이후 스크래핑을 GCP Cloud Run 워커로
#: 옮기면서(상시 무료 한도 월 200만 요청) 부하 한도가 훨씬 여유로워져
#: 30으로 다시 올렸다 - 최악의 경우(6명이 서로 안 겹치는 배를 각자 30척씩
#: 감시)여도 회차당 최대 180개 대상인데, 등록된 배 자체가 100척이 안 돼
#: 실제로는 이 숫자를 못 넘고, 순차 수집(배당 1~2초)으로도 30분 주기 안에
#: 충분히 끝난다.
MAX_WATCHES_PER_SUBSCRIBER = 30
